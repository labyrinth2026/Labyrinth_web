  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-2xl shadow-sm p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-xl font-bold font-grotesk text-[#0B1F63]">Content Manager</h1>
          <p className="text-[#667085] text-sm mt-0.5">Manage website content via simulated backend.</p>
        </div>
        <button onClick={openAddModal} className="flex items-center gap-2 px-4 py-2 bg-[#0B1F63] text-white text-sm font-semibold rounded-xl hover:bg-[#071545] transition-colors">
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-2xl shadow-sm overflow-hidden">
        <div className="flex border-b border-[#E5E7EB]">
          ...
          <button onClick={loadData} className="ml-auto px-4 py-4 text-[#8c97a8] hover:text-[#0B1F63] transition-colors">
        ...
        {isLoading ? (
          <div className="p-12 flex flex-col items-center gap-2 text-[#8c97a8]">
            <RefreshCw size={24} className="animate-spin text-[#0B1F63]" />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-[#E5E7EB]">
                {data.map((item: any) => (
                  <tr key={item.id} className="hover:bg-[rgba(11,31,99,0.03)]/30 transition-colors text-sm">
                    <td className="p-4 text-[#0B1F63] font-semibold">{item.title || item.name}</td>
                    {activeTab === 'events' && (
                    ...
                    {activeTab === 'verticals' && (
                      <td className="p-4">
                        <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${item.category === 'tech' ? 'bg-[rgba(11,31,99,0.03)] text-[#0B1F63] border border-[rgba(11,31,99,0.07)]' : 'bg-green-50 text-green-600 border border-green-200'}`}>
                          {item.category}
                        </span>
                      </td>
                    )}
                    <td className="p-4 text-right">
                      <div className="flex justify-end gap-1.5">
                        <button onClick={() => openEditModal(item)} className="p-1.5 text-[#8c97a8] hover:text-[#0B1F63] hover:bg-[rgba(11,31,99,0.03)] rounded-lg transition-colors"><Edit2 size={14} /></button>
        ...
        <div className="fixed inset-0 z-50 bg-[#0B1F63]/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl border border-[#E5E7EB] w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-[#E5E7EB]">
              <h2 className="font-bold text-[#0B1F63]">{editingItem.id ? 'Edit' : 'Add'} {activeTab === 'events' ? 'Event' : 'Vertical'}</h2>
              <button onClick={() => setShowModal(false)} className="text-[#8c97a8] hover:text-[#0B1F63] transition-colors"><X size={18} /></button>
            </div>
            ...
            <div className="flex justify-end gap-3 p-6 border-t border-[#E5E7EB]">
              ...
              <button onClick={handleSave} disabled={isSaving} className="flex items-center gap-2 px-5 py-2 bg-[#0B1F63] text-white text-sm font-semibold rounded-xl hover:bg-[#071545] transition-colors disabled:opacity-50">
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 bg-[#0B1F63]/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl border border-red-100 w-full max-w-sm p-6 text-center">
            ...
            <h3 className="font-bold text-[#0B1F63] mb-2">Remove Item?</h3>
            <p className="text-[#667085] text-sm mb-6">This action cannot be undone.</p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 px-4 py-2 text-sm font-semibold text-[#667085] border border-[#E5E7EB] rounded-xl hover:bg-[rgba(11,31,99,0.03)] transition-colors">Cancel</button>
