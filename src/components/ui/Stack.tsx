...
export default function Stack() {
  ...
  return (
    <div
      ...
      style={{ height: `${totalCards * 60 + 80}vh` }}
    >
      {/* ── Sticky Centered Viewport Container ── */}
      <div className="sticky top-20 sm:top-24 h-[75vh] min-h-[460px] max-h-[560px] max-w-3xl mx-auto px-2 sm:px-4 flex flex-col justify-center items-center">
        
        {/* Step Indicator Header */}
        <div className="w-full flex items-center justify-between mb-3 px-2 text-xs font-mono tracking-widest text-slate-400 uppercase font-bold">
          <span className="flex items-center gap-1.5 text-slate-300">
            <Layers size={14} className="text-[#CD0000]" />
            VERTICAL {activeIndex === 0 ? 'OVERVIEW' : `${activeIndex} OF ${totalCards - 1}`}
          </span>
          <span className="text-[#CD0000] font-bold">
            {VERTICALS_DATA[activeIndex].name}
          </span>
        </div>

        {/* ── Card Stack Viewport Frame ── */}
        <div className="relative w-full h-[400px] sm:h-[430px] rounded-[32px] overflow-hidden shadow-2xl">
        ...
        <div className="mt-3 flex items-center gap-1 text-[11px] text-slate-400 font-medium">
          <span>Scroll down to reveal next vertical card</span>
