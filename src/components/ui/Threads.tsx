...
function parseColor(c: [number, number, number] | string | undefined): [number, number, number] {
  if (typeof c === 'string') {
    const hex = c.replace('#', '');
    if (hex.length === 6) {
  ...
  // Default CHRIST Labyrinth Red (#CD0000) -> [205/255, 0, 0]
  return [205 / 255, 0, 0];
}
...
const Threads: React.FC<ThreadsProps> = ({
  ...
  const propsRef = useRef({ color: parsedColor, amplitude, distance, enableMouseInteraction });
  propsRef.current = { color: parsedColor, amplitude, distance, enableMouseInteraction };
  ...
  useEffect(() => {
    ...
    // Cap long side and devicePixelRatio on mobile/high-DPI to ensure ultra-smooth performance
    ...
    return () => {
      ...
      if (renderer && container && container.contains(gl.canvas)) {
