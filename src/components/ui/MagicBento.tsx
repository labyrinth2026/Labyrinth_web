...
const DEFAULT_GLOW_COLOR = '205, 0, 0';
...
const BentoCardMedia: React.FC<{
  src: string;
  rotation?: number;
  title: string;
}> = ({ src, rotation = 0, title }) => {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      for (const entry of entries) {
        ...
        if (width && height) {
          setScale(Math.max(width / height, height / width));
        }
      }
    });
  ...
  const isRotated = rotation === 90 || rotation === 270;
  const transformStyle = `translate(-50%, -50%) rotate(${rotation}deg)${isRotated ? ` scale(${scale})` : ''}`;
  const isVideo = src.endsWith('.mp4') || src.endsWith('.webm');

  return (
    <div ref={containerRef} className="magic-bento-img-container">
      {isVideo ? (
        <video
          src={src}
...
const BentoCardGrid: React.FC<{
  ...
  <div
    className="magic-bento-section select-none relative"
    style={{
    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef} glowColor={glowColor}>
        {items.map((item, index) => {
          const baseClassName = `magic-bento-card ${enableBorderGlow ? 'border-glow' : ''}`;
          ...
          return (
            ...
            >
              {/* Media background component with rotation auto-scaling */}
              {item.image && (
                <BentoCardMedia
                  src={item.image}
              ...
              {/* Zoom icon to indicate interactive lightbox zoom capability */}
