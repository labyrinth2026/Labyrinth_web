...
const AboutPage: React.FC = () => {
  ...
  const features = [
    { icon: BookOpen, title: 'Hands-on Learning', desc: 'Move beyond theory with practical workshops and projects.', color: '#005BAC' },
    { icon: Briefcase, title: 'Industry Connections', desc: 'Network with professionals from top tech companies.', color: '#1a6fc0' },
    { icon: Users, title: 'Peer Community', desc: 'Find co-founders, mentors, and lifelong friends.', color: '#0369a1' },
    { icon: Zap, title: 'Career Growth', desc: 'Build a portfolio that makes your resume stand out.', color: '#1a8fc4' }
  ];
  ...
  const TimelineItem = ({ milestone, index }: { milestone: any; index: number }) => {
    ...
    return (
      <div ref={ref} className={`relative flex items-center justify-between md:justify-normal w-full mb-10 ${isEven ? 'md:flex-row-reverse' : ''}`}>
        {/* Center Dot */}
        <div className="hidden md:block absolute left-1/2 -translate-x-1/2 w-4 h-4 rounded-full bg-[#005BAC] border-4 border-white z-10 shadow-md" />
        ...
        <div className="w-full md:w-5/12">
          ...
          >
            <div className="bg-white border border-blue-100 rounded-2xl shadow-sm p-6 relative">
              {/* Year Badge */}
              <span className="absolute -top-3 left-6 bg-[#005BAC] text-white px-4 py-0.5 rounded-full text-sm font-bold shadow">
                {milestone.year}
              </span>
              <h3 className="text-lg font-bold text-[#1a2c4a] mb-1 mt-2">{milestone.title}</h3>
              <p className="text-[#4b6080] text-sm leading-relaxed">{milestone.desc}</p>
            </div>
          </motion.div>
        </div>
      </div>
  ...
  return (
    <PageWrapper>
      {/* Hero Banner */}
      <section className="relative py-20 bg-gradient-to-br from-[#EAF4FF] to-white overflow-hidden">
        <div className="absolute right-0 top-0 w-96 h-96 bg-[#005BAC]/5 rounded-full blur-[80px] pointer-events-none" />
        <div className="container mx-auto px-6 max-w-5xl text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <h1 className="font-grotesk text-5xl md:text-6xl font-bold mb-6 text-[#1a2c4a]">
              About <span className="text-[#005BAC]">LABYRINTH</span>
            </h1>
            <p className="text-lg text-[#4b6080] max-w-3xl mx-auto leading-relaxed mb-4">
            ...
            <p className="text-xl font-bold text-[#005BAC] max-w-3xl mx-auto leading-relaxed mt-6">
        ...
        <div className="container mx-auto px-6 max-w-6xl">
          <div className="grid md:grid-cols-2 gap-8">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <div className="bg-white border border-blue-100 rounded-2xl shadow-sm p-10 h-full hover:shadow-lg hover:shadow-[#005BAC]/10 transition-shadow">
                <div className="w-14 h-14 rounded-2xl bg-[#EAF4FF] flex items-center justify-center text-[#005BAC] mb-6">
                  <Target size={28} />
                </div>
                <h2 className="text-2xl font-bold text-[#1a2c4a] mb-3 font-grotesk">Our Mission</h2>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.15 }}>
              <div className="bg-[#005BAC] rounded-2xl shadow-md p-10 h-full">
        ...
        <div className="container mx-auto px-6 max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((feature, i) => (
              ...
              >
                <div className="bg-white border border-blue-100 rounded-2xl shadow-sm p-6 h-full text-center group hover:shadow-lg hover:shadow-[#005BAC]/10 hover:-translate-y-1 transition-all">
                  <div className="w-12 h-12 mx-auto rounded-xl bg-[#EAF4FF] flex items-center justify-center text-[#005BAC] mb-4 group-hover:bg-[#005BAC] group-hover:text-white transition-all">
                    <feature.icon size={22} />
                  </div>
                  <h3 className="text-base font-bold text-[#1a2c4a] mb-2">{feature.title}</h3>
                  <p className="text-sm text-[#4b6080] leading-relaxed">{feature.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-6 max-w-5xl">
          ...
          <div className="relative mt-12">
            {/* Center Line */}
            <div className="hidden md:block absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-[#005BAC] to-[#D6EBFF]" />
