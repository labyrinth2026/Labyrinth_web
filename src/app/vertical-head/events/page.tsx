...
const EMPTY_EVENT = (verticalId: string): Event => ({
  title: '', description: '', date: '', category: 'Workshop', status: 'upcoming', featured: false, verticalId
});

export default function VerticalEventsPage() {
  const { user } = useAuth();
  ...
  const loadEvents = async () => {
    if (user?.verticalId) {
      ...
      try {
        ...
        const vertEvents = (data || []).filter((e: any) => e.verticalId === user.verticalId);
        setEvents(vertEvents);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const openAddModal = () => {
    if (user?.verticalId) {
      setEditingEvent(EMPTY_EVENT(user.verticalId));
      setShowModal(true);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-[#E5E7EB] rounded-2xl shadow-sm p-6 flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold font-grotesk text-[#CD0000]">Vertical Events</h1>
          <p className="text-[#667085] text-sm mt-0.5">Manage and host events/hackathons for your vertical.</p>
        </div>
        <button 
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="p-5 space-y-4 text-left">
                <div>
                  ...
                  <input type="text" required value={editingEvent.title} onChange={e => setEditingEvent({ ...editingEvent, title: e.target.value })} className={inputClass} placeholder="e.g. AI Hackathon 2026" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-[#8c97a8] block mb-1">Description</label>
                  <textarea rows={3} value={editingEvent.description} onChange={e => setEditingEvent({ ...editingEvent, description: e.target.value })} className={inputClass} placeholder="Details about registration, prizes, venue..." />
                </div>
