...
export default function CoreDashboardPage() {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      if (user?.committeeId) {
        try {
          const tasksData: any = await fetchFromSheet('getCommitteeTasks', { committeeId: user.committeeId });
          setTasks(tasksData || []);

          const resData: any = await fetchFromSheet('getCommitteeResources', { committeeId: user.committeeId });
          setResources(resData || []);

          const evts: any = await fetchFromSheet('getEvents');
          setEvents((evts || []).filter((e: any) => e.committeeId === user.committeeId));

          const anns: any = await fetchFromSheet('getAnnouncements');
          setAnnouncements((anns || []).filter((a: any) => a.targetType === 'all' || (a.targetType === 'committee' && a.targetId === user.committeeId)));
        } catch (e) {
  ...
  const stats = [
    { label: 'Pending Tasks', value: pendingTasks.toString(), icon: CheckSquare, color: '#CD0000', bg: 'rgba(205, 0, 0, 0.03)' },
    { label: 'Committee Events', value: events.length.toString(), icon: Calendar, color: '#16a34a', bg: '#F0FDF4' },
    { label: 'Resources Shared', value: resources.length.toString(), icon: FolderOpen, color: '#7c3aed', bg: '#F5F3FF' },
  ];
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 text-xs font-semibold uppercase tracking-wider">
              Manage your committee's workflow, events, and resources.
            </p>
          </div>
              ...
              announcements.map((ann, i) => (
                <div key={i} className="border-b border-slate-100 pb-3 last:border-b-0 last:pb-0">
                  <span className="text-[8px] font-bold text-[#CD0000] uppercase tracking-wider bg-[#CD0000]/5 px-2 py-0.5 rounded-full inline-block mb-1">
                    {ann.targetType === 'all' ? 'All Club' : 'Committee Only'}
                  </span>
