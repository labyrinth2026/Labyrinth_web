    ...
    const loadMembers = async () => {
      if (user?.committeeId) {
        try {
          ...
          const commAssignments = res.committee || [];
          
          // Filter assignments for my committee
          const myAssignments = commAssignments.filter((a: any) => a.committeeId === user.committeeId);
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-[#E5E7EB] rounded-2xl shadow-sm p-6">
        <h1 className="text-xl font-bold font-grotesk text-[#CD0000]">Committee Members</h1>
        <p className="text-[#667085] text-sm mt-0.5">List of leaders and heads assigned to this committee.</p>
      </div>
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-2xl shadow-sm overflow-hidden">
        {members.length > 0 ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-[#667085]">Ask the administrators to assign members to your committee.</p>
          </div>
        )}
      </div>
    </div>
