...
export default function CommitteeResourcesPage() {
  const { user } = useAuth();
  ...
  const loadResources = async () => {
    if (user?.committeeId) {
      setLoading(true);
      try {
        const data: any = await fetchFromSheet('getCommitteeResources', { committeeId: user.committeeId });
        setResources(data || []);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await fetchFromSheet('addCommitteeResource', { committeeId: user?.committeeId, ...editingRes });
      setShowModal(false);
      loadResources();
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetchFromSheet('deleteCommitteeResource', { id });
      loadResources();
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-[#E5E7EB] rounded-2xl shadow-sm p-6 flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold font-grotesk text-[#CD0000]">Committee Resources</h1>
          <p className="text-[#667085] text-sm mt-0.5">Share assets, references, and files with your committee members.</p>
        </div>
        <button 
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
              ...
              <tbody className="divide-y divide-[#E5E7EB]">
                {resources.map((res) => (
                  <tr key={res.id} className="hover:bg-[rgba(205, 0, 0, 0.03)]/30 transition-colors">
                    ...
                    <td className="p-4">
                      <span className="bg-purple-50 text-purple-600 px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider border border-purple-100">
                        {res.type}
                      </span>
                    </td>
                    <td className="p-4">
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-[#667085]">There are no operational resources posted yet. Add one to get started.</p>
          </div>
        )}
      </div>
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="p-5 space-y-4 text-left">
                <div>
                  ...
                  <input type="text" required value={editingRes.title} onChange={e => setEditingRes({ ...editingRes, title: e.target.value })} className={inputClass} placeholder="e.g. Design Guideline Kit" />
                </div>
                <div>
