...
export default function CommitteeAnnouncementsPage() {
  ...
  const [targetType, setTargetType] = useState<'all' | 'committee'>('committee');
  ...
  const loadAnnouncements = async () => {
    if (user?.committeeId) {
      ...
      try {
        ...
        // Filter: targetType is all, or targetId is my committee
        const filtered = (list || []).filter((a: any) => 
          a.targetType === 'all' || (a.targetType === 'committee' && a.targetId === user.committeeId)
        );
        setAnns(filtered);
      } catch (e) {
        console.error(e);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await fetchFromSheet('addAnnouncement', {
        ...
        content,
        targetType,
        targetId: targetType === 'committee' ? user?.committeeId : undefined
      });
      setTitle('');
      setContent('');
      setShowModal(false);
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-[#E5E7EB] rounded-2xl shadow-sm p-6 flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold font-grotesk text-[#CD0000]">Committee Announcements</h1>
          <p className="text-[#667085] text-sm mt-0.5">Post and view updates or warnings for the club or committee.</p>
        </div>
        <button 
              ...
              <div className="space-y-2">
                <div className="flex gap-2 items-center">
                  <span className={`text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border ${
                    ...
                    {ann.targetType === 'all' ? 'All Club Notice' : 'Committee Internal'}
                  </span>
            ...
            <form onSubmit={handleCreate}>
              ...
              <div className="p-5 space-y-4 text-left">
                <div>
                  ...
                  <input type="text" required value={title} onChange={e => setTitle(e.target.value)} className={inputClass} placeholder="e.g. Mandatory meeting at 3 PM today" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <select value={targetType} onChange={e => setTargetType(e.target.value as any)} className={inputClass}>
                    <option value="committee">Committee Internal Only</option>
