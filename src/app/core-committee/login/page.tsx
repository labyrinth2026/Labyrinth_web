...
export default function CoreCommitteeLoginPage() {
  ...
  useEffect(() => {
    if (user && user.role === 'CORE_HEAD') {
      router.push('/core-committee/dashboard');
    }
  }, [user, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await login(email, password, 'core');
    if (result.success) {
      router.push('/core-committee/dashboard');
    } else {
      ...
      <div className="relative z-10 w-full max-w-md mx-4">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight font-grotesk">
            COMMITTEE <span className="text-[#CD0000]">LOGIN</span>
          </h1>
          <p className="text-slate-400 text-xs mt-2">
            Portal for Core Committee Heads to manage their designated committee.
          </p>
        </div>
